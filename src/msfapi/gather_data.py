...
class GetStats:
    ...
    tpf = {
        'Coors Field':1.332,
        'Globe Life Park in Arlington':1.215,
        'Chase Field':1.202,
        'Comerica Park':1.169,
        'Wrigley Field':1.131,
        'Oakland Coliseum':1.103,
        'Target Field':1.095,
        'Miller Park':1.094,
        'Citizens Bank Park':1.073,
        'Nationals Park':1.057,
        'Fenway Park':1.026,
        'Oriole Park at Camden Yards':1.026,
        'Great American Ball Park':1.020,
        'Yankee Stadium':1.019,
        'Guarenteed Rate Field':1.003,
        'SunTrust Park':0.976,
        'Progressive Field':0.971,
        'Dodger Stadium':0.971,
        'Angel Stadium of Anaheim':0.949,
        'PNC Park':0.948,
        'Rogers Centre':0.946,
        'Kauffman Stadium':0.926,
        'T-Mobile Park':0.917,
        'Tropicana Field':0.902,
        'Busch Stadium':0.889,
        'Citi Field':0.858,
        'Oracle Park':0.850,
        'Marlins Park':0.848,
        'Petco Park':0.828,
        'Minute Maid Park':0.826
    }
    ...
    def get_game(self, season):
        games = pd.DataFrame(columns=['date', 'time', 'id', 'park', 'fir_label'])
        ...
        for game in season_games['games']:
            ...
            games = games.append({'date': date, 'time': time, 'id': id, 'park': park, 'fir_label': fir_label}, ignore_index=True)   
        return games
            ...
            for player in sps['playerStatsTotals']:
                ...
                try:
                    ...
                    for idx in range(len(lineups[batter]['game_id'])):
                        ...
                        data[game_id][lineups[batter]['title'][idx]]['HR'] = (player['stats']['batting']['homeruns'] / player['stats']['batting']['atBats'])
                except:
                    continue
        ...
        for player in sps['playerStatsTotals']:
            ...
            try:
                ...
                for idx in range(len(lineups[batter]['game_id'])):
                    ...
                    data[game_id][lineups[batter]['title'][idx]]['prev_HR'] = (player['stats']['batting']['homeruns'] / player['stats']['batting']['atBats'])
            except:
                continue
    ...
    def get_average_bo_stats(self, game):
        ...
        away_hr_count = 0
        away_hr_total = 0
        away_prev_hr_total = 0
        home_prev_hr_count = 0
        home_prev_hr_total = 0
        away_prev_hr_count = 0
        home_left_hand = 0
        home_right_hand = 0
        away_left_hand = 0
        away_right_hand = 0
        ...
        for key in home_prev_key_list:
            if game[key]['prev_HR'] > -1:
                home_prev_hr_total += game[key]['prev_HR']
                home_prev_hr_count += 1
        
        for key in away_prev_key_list:
            if game[key]['prev_HR'] > -1:
                away_prev_hr_total += game[key]['prev_HR']
                away_prev_hr_count += 1
        ...
        if home_hr_count == 0:
            ...
            home_hr = home_hr_total / home_hr_count
        if away_count == 0:
            away_ops = -1
        else:
        ...
        if away_hr_count == 0:
            ...
            away_hr = away_hr_total / away_hr_count
        ...
        if home_prev_hr_count == 0:
            home_prev_hr = -1
        else:
            home_prev_hr = home_prev_hr_total / home_prev_hr_count
        if away_prev_hr_count == 0:
            away_prev_hr = -1
        else:
            away_prev_hr = away_prev_hr_total / away_prev_hr_count

        return home_hr, away_hr, home_prev_hr, away_prev_hr, home_ops, away_ops, home_prev_ops, away_prev_ops, home_left_hand, home_right_hand, away_left_hand, away_right_hand
    ...
    for idx, date in enumerate(dates[1:]):
        ...
        for num, game in day_games.iterrows():
            ...
            try:
                ...
                data[game['id']]['home_BO1']['HR'] = -1
                data[game['id']]['home_BO2']['HR'] = -1
                data[game['id']]['home_BO3']['HR'] = -1
                data[game['id']]['home_BO4']['HR'] = -1
                data[game['id']]['home_BO5']['HR'] = -1
                data[game['id']]['away_BO1']['HR'] = -1
                data[game['id']]['away_BO2']['HR'] = -1
                data[game['id']]['away_BO3']['HR'] = -1
                data[game['id']]['away_BO4']['HR'] = -1
                data[game['id']]['away_BO5']['HR'] = -1
                ...
                data[game['id']]['home_BO1']['prev_HR'] = -1
                data[game['id']]['home_BO2']['prev_HR'] = -1
                data[game['id']]['home_BO3']['prev_HR'] = -1
                data[game['id']]['home_BO4']['prev_HR'] = -1
                data[game['id']]['home_BO5']['prev_HR'] = -1
                data[game['id']]['away_BO1']['prev_HR'] = -1
                data[game['id']]['away_BO2']['prev_HR'] = -1
                data[game['id']]['away_BO3']['prev_HR'] = -1
                data[game['id']]['away_BO4']['prev_HR'] = -1
                data[game['id']]['away_BO5']['prev_HR'] = -1
                ...
                continue
            try:
                data[game['id']]['park'] = stats.tpf[game['park']]
            except(KeyError):
                data[game['id']]['park'] = 1.0
        ...
        for idx, game in day_games.iterrows():
            try:
                ...
                continue
            home_hr, away_hr, home_prev_hr, away_prev_hr, home_ops, away_ops, home_prev_ops, away_prev_ops, home_left_hand, home_right_hand, away_left_hand, away_right_hand, = stats.get_average_bo_stats(data[game['id']])
            ...
            games_data = games_data.append({
                ...
                'home_curr_hr': home_hr,
                'home_prev_hr': home_prev_hr,
                'away_curr_hr': away_hr,
                'away_prev_hr': away_prev_hr,
                ...
                'park_rating': data[game['id']]['park'],
                'fir_result': game['fir_label']
            }, ignore_index=True)
        games_data.to_csv('data/' + season['title'] + 'LabeledRawNew.csv', mode='a', sep=',', index=False, header=False)
